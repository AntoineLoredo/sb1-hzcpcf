  ...
  products.forEach(product => {
    product.variants.forEach(variant => {
      rows.push([
        ...
        variant.isBundle ? 'Yes' : 'No',
        variant.isFromRawMaterial ? 'Yes' : 'No',
        variant.moq,
...
export async function parseProductsCSV(file: File): Promise<Product[]> {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const variant: ProductVariant = {
      ...
      dimensions: {
        ...
        unit: data['Dimension unit'] as 'cm' | 'in'
      },
      isBundle: data['IsBundle'].toLowerCase() === 'yes',
      isFromRawMaterial: data['IsFromRawMaterial'].toLowerCase() === 'yes',
